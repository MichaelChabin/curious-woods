...
**A Small Brain Atlas (12 Sep, Claude Code; placed on the home page on Michael's
instruction).** `active/brain-atlas.html` — a short reading, then Michael's two brain
watercolours, outside and inside, with twelve named places that light and speak one
sentence when tapped, and six things the brain does (seeing three triangles, reading a
word, hearing a beat, tracing the star, remembering, touching something hot) each drawn
as a signal travelling its road, steppable, with a slow-to-real slider and a counter of
brain-time in ms. It is the atlas the Brain bench spec describes as a textbook diagram the
bench must never show — and it is a page in its own right, standing beside the labs, not
the bench. Region ids and coordinates are the spec's, so the bench can be cut from it.
Moved from `experiments/` unchanged; both paintings are inline base64 (4.1 MB), no shared
scripts, no `../` paths, nothing to check.

- **Authority:** `cw-deploys/active/brain-atlas.html` · `20-SPECS/Spec-Brain-Bench.md`
  (the region vocabulary) · `claude/Brain-Series-Map.md` (the stories the six signals
  come from).
- **Two things left standing, not patched:** the atlas carries **no `CW_VERSION` stamp**,
  against the standing method, because the instruction was to change nothing inside the
  file; and its workbench — the two paintings, the overlay SVG, the check renders,
  `make_overlay.py` — still sits at `experiments/brain/`, public by folder and linked
  from nowhere. Both are one decision each.
- **Next action:** Michael says whether the stamp goes in (one line at the foot of the
  file) and whether `experiments/brain/` moves to `prototypes/`. Then the Brain bench
  v0, per the spec, cut from this atlas's region tables.
