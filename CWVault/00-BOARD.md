...
**The gallery — built 20 Sep (Claude Code, from `20-SPECS/Spec-Gallery.md`), awaiting Michael's look before it commits.**
...
for want of an icon:** Glass Geometry, Glass Multiplication, The Necker Cube; the labs are
on `labs.html`, the cube on the experiments index. A finished story (slug in
...
for the two labs and the Necker Cube, so they can hang. (7) Whether the three mocks stay in
...
land in the same commit). Then icons for the labs and the cube, and the story page's
end-of-text record.**

**Maps — the bench is built and recoloured, not yet committed (18 Sep, Claude Code, from `claude/Spec-Maps.md`).**
...
**Next action: Michael opens `experiments/maps/map-bench.html`, judges the two pictures and
the window, and says yes or what to change; then commit and push.** Open, and not decided
here: lakes are land in the grid (Lake Geneva is a dot on paper); where `map.js` lives once
a story calls it; the spec's own five *Not yet decided* questions.
