...
- **Three things the file lacks, each one line at its top, reported not fixed** (the
  instruction was to change nothing inside it): no doctype and no `<meta charset>`, so
  its em-dashes depend on the server's header (Netlify sends UTF-8; a local server may
  not); no `<link rel="icon">`, so Safari's tab shows a letter where About Your Brain shows
  the brain; no `CW_VERSION` stamp.
- **Left for later, as instructed:** `experiments/star-in-a-mirror.html`, the practice
  version of the puzzle — the site has no practice area yet, and the story's inscription
  already queues `mirror-star` for one. It sits untracked in `experiments/`.
- **Untracked files in the publish directory, not mine to remove:** the story's source
  copy `experiments/trace-prototype-star-story.html`, `experiments/star-in-a-mirror.html`,
  and `experiments/brain-atlas.html` — a re-export identical to the 13 Sep revision that
  ships as About Your Brain. A `git add -A` would publish all three.
- **Tension, reported:** this page and `experiments/story-learned-without-knowing.html`
  ...
  call.
- **Next action:** Michael's word on the three lines. Then a practice area for Star in a
  Mirror; then icons for the two Glass cards.
