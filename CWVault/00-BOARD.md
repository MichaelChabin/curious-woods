...
- **Three things left standing, all waiting on Michael's word about the file:** the tab
  icon line above; the **`CW_VERSION` stamp** the revision lost (the instruction was to
  change nothing inside it, as on the 12th); and its workbench — the two paintings, the
  overlay SVG, the check renders, `make_overlay.py` — still at `experiments/brain/`,
  public by folder and linked from nowhere.
- **Next action:** Michael says yes or no to the two lines in the file (icon, stamp) and
  whether `experiments/brain/` moves to `prototypes/`. Then icons for the two Glass
  cards, so the home page is one language; then the Brain bench v0, per the spec, cut
  from this atlas's region tables.
