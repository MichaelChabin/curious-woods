...
**Professor Necker's Drawing — built 25 Sep (Claude Code), stage 2 of Publishing-a-Story; not committed; awaiting Michael's read and his yes.**
`cw-deploys/experiments/professor-neckers-drawing.html`, from `claude/Story-Professor-Neckers-Drawing.md`
Draft 2.3 and the mock-up's geometry. Registered in MANIFEST; not in `gallery.json`.
**Next action:** Michael reads it at its own address, then says yes (commit and push) or sends
fixes. **Open, named, not decided:** (1) the story's own words twice say where a picture is
("the drawing at the top of this story"), which the 24 Sep rule forbids — kept verbatim, his
call; (2) the named map crop `britain-and-geneva` is portrait (about 1,240 px tall at full
width) where Story-Pattern wants a World map wide and shallow; (3) the practice list now exists
(`practice.html`, `js/remember.js`) and the gallery's ensō now opens it (Michael, 25 Sep; it
went to Trace); the two older stories keep their inline copies;
Rulings-Sept-2026 still says the queue does not exist — Michael is telling the manager chat;
Safari may clear the list after about seven days without a visit unless the site is on the home
screen — a home-screen invitation is proposed, not built; (4) `Story-A-Very-Strange-Cube.md`, which the prompt named for the World's event list,
is not on disk; the World was built from the list in the prompt; (5) the frame's grey-blue
needs a hex when the story is hung; (6) in *More*, the pictures are taller than their short
paragraphs, so the last two sit a paragraph low.
