...
**Three at a Glance — built, read once, changed (16 Sep, Claude Code).**
`cw-deploys/experiments/Three_at_Glance.html`, from `claude/Story-Three-at-a-Glance.md`
(now Draft 5, matching the page), on the model of the star story's page. Not in the gallery,
not on the experiments index. Michael's first read changed it, so that an anxious or
uncertain child isn't penalised: *Ready* a second before the dots, a reveal and a fade
after, one dot size in Payne's grey, *preview* and *stop*, new opening instructions, and
*tap on 2* to *tap on 7* with a by-number graph in place of *tap on six*. **Next action:
Michael reads it again, on the iPad and the Mac.** Detail in MANIFEST under
`experiments/`. Open, and named in the story's notes: text in *What the experiment was for*
and *Six, in two colours* that still assumes the old twos-fours-fives and tap-on-six
experiments; the knob sentence's new wording; equal dots making total grey a cue for
number; no References section; the colour patch placed by eye; the 200 ms flash in the red
and triangle searches. The filename departs from the folder's lower-case-and-hyphens
habit, as instructed.
