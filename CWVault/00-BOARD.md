...
- **Authority:** `cw-deploys/active/brain-atlas.html` · `20-SPECS/Spec-Brain-Bench.md`
  ...
  come from).
- **Pushed 12 Sep as a demo**, stamped on Michael's word (the one line inside the file
  that is not the atlas's own). One thing left standing: its workbench — the two
  paintings, the overlay SVG, the check renders, `make_overlay.py` — still sits at
  `experiments/brain/`, public by folder and linked from nowhere.
- **Next action:** Michael says whether `experiments/brain/` moves to `prototypes/`.
