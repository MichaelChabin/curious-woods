...
**Three at a Glance — built, read, changed twice (16 Sep, Claude Code).**
`cw-deploys/experiments/Three_at_Glance.html`, from `claude/Story-Three-at-a-Glance.md`
(Draft 5, matching the page). Not in the gallery, not on the experiments index. Michael's
reads changed it twice. First, so an anxious or uncertain child isn't penalised: *Ready* a
second before the dots, a reveal and a fade after, one dot size in Payne's grey, *preview*
and *stop*, new opening instructions, *tap on 2* to *tap on 7* with a by-number graph.
Second, the layout: the experiments now sit beside the text that names them, titled
Experiment 1 to 4, all present at once (a local alternative he preferred; the Pictures ruling
is amended for this page). **Next action: Michael reads it again, on the iPad and in Safari
on the Mac.** Detail in MANIFEST under `experiments/`. Open, and named in the story's notes:
text in *What the experiment was for* and *Six, in two colours* that still assumes the old
experiments; the knob sentence's new wording; equal dots making total grey a cue for number;
"on the left" in the text on narrow screens, where the experiments sit in the story; no
References section; the colour patch placed by eye; the 200 ms flash in Experiment 4. Which
picture rule is the default for new stories is not yet ruled. The filename departs from the
folder's lower-case-and-hyphens habit, as instructed.
